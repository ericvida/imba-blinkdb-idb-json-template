...
let encKey = null;
  ...
  let aencrypted = acipher.update(JSON.stringify(newJson,null,4));
...
if (fixDB.size === 0) fs.writeFileSync('./cheffydb.json', "{}");
 ...
 s[id] = value
 encKey ? updateEncryptedDB(s) : fs.writeFileSync('./cheffydb.json', JSON.stringify(s,null,4));
 return value
  },
...
if (fixDB.size === 0) fs.writeFileSync('./cheffydb.json', "{}");
    ...
    try {
	   ...
	   delete obj[id]
	   encKey ? updateEncryptedDB(JSON.stringify(obj,null,4)) : fs.writeFileSync('./cheffydb.json', JSON.stringify(obj,null,4));
	   return true;
    } catch (err) {
      return false;
    }
...
if (fixDB.size === 0) fs.writeFileSync('./cheffydb.json', "{}");
 ...
 fs.writeFileSync('./cheffydb.json', JSON.stringify(parsed,null,4));
			}
    })
	  } else {
...
if(fs.existsSync(backupfileloc) === false) {
 ...
 parsed[dbvalues.ID] = dbvalues.data
 fs.writeFileSync(backupfileloc, JSON.stringify(parsed,null,4));
    });
   return true;
  },
